...
import { DEFECT_TYPE_LABEL } from "@/lib/constants";
...
function SkeletonLine({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "h-4 animate-pulse rounded-lg bg-tenunara-teal/10",
        className,
      )}
    />
  );
...
export function AIResultDisplay({
  ...
  const colorName =
    result.ai_dominant_color?.split(";")[1] || result.ai_dominant_color || "-";

  const fields: { label: string; value: string }[] = [
    { label: "Warna Dominan", value: colorName },
    { label: "Pola", value: result.ai_pattern || "-" },
    { label: "Estimasi Dimensi", value: result.ai_size_range || "-" },
  ];
    ...
    <div className="space-y-4 rounded-3xl bg-white p-6 shadow-sm">
      ...
      {result.ai_reasoning && (
        <div className="rounded-xl bg-tenunara-mint/30 p-3">
          <p className="text-xs font-medium text-tenunara-teal mb-1">
            Alasan AI:
          </p>
      ...
      {result.defects && result.defects.length > 0 && (
        <div>
          <p className="text-xs font-medium text-tenunara-teal mb-2">
            Cacat Terdeteksi:
          </p>
            ...
            {result.defects.map((d, i) => (
              ...
              >
                ...
                <span className="text-xs text-tenunara-teal">
                  {d.defect_percentage}% area (confidence:{" "}
