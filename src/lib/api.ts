...
export function productToListing(
  ...
  const isProductWithFabric = "fabric_name" in product || "umkm_name" in product
...
export async function fetchMyProducts(umkmId: string): Promise<ListingWithSeller[]> {
...
export async function fetchProductById(id: string): Promise<ProductRow & { fabric_name: string; defects: ProductDefectDetail[] }> {
  const res = await fetchJson<SingleResponse<ProductRow & { fabric_name: string; defects: ProductDefectDetail[] }>>(
    `/api/products/${id}`,
  )
  return res.data
}
...
export async function fetchDashboardProducts(params?: {
  ...
  if (params?.fabric_type_id) searchParams.set("fabric_type_id", String(params.fabric_type_id))
  ...
  const res = await fetchJson<any>(`/api/products/dashboard${qs ? `?${qs}` : ""}`)
  return res
}
...
export async function createProduct(data: CreateProductRequest): Promise<ProductRow> {
...
export async function updateProduct(id: string, data: UpdateProductRequest): Promise<ProductRow> {
  const res = await fetchJson<SingleResponse<ProductRow>>(`/api/products/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
  })
  return res.data
...
export async function analyzeProductImages(imageCount: number): Promise<AIAnalysisResult> {
...
export async function uploadProfilePhoto(imageBase64: string): Promise<{ url: string }> {
  const res = await fetchJson<{ url: string }>("/api/auth/upload-profile-photo", {
    method: "POST",
    body: JSON.stringify({ foto_profil_base64: imageBase64 }),
  })
  return res
}
